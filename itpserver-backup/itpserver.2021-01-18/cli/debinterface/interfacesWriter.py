...
class InterfacesWriter:
    ...
    _prepFields = ['pre-up', 'up', 'down', 'post-down', 'post-up']
    _bridgeFields = ['ports', 'fd', 'hello', 'maxage', 'stp']
    _bondFields = ['slaves','mode', 'miimon', 'downdelay', 'updelay', 'mtu']
...
#        self._write_unknown(interfaces, adapter, ifAttributes)
    ...
    def _write_bond(self, interfaces, adapter, ifAttributes):
        ...
        for field in self._bondFields:
            try:
                ...
                if field == 'slaves': 
                    d = dict(varient=field, value=ifAttributes['bond-opts'][field])
                elif field == 'mtu':
                    d = dict(varient=field, value=ifAttributes['bond-opts'][field])
                else:
                    d = dict(varient="bond-" + field, value=ifAttributes['bond-opts'][field])
                interfaces.write(self._cmd.substitute(d))
    ...
    def _write_callbacks(self, interfaces, adapter, ifAttributes):
        ''' Write the up, down, pre-up, post-up and post-down clauses. '''
