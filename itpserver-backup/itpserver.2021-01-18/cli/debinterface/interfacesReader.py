...
class InterfacesReader:
    ...
    def _parse_details(self, line):
        if line[0].isspace() is True:
            ...
            elif sline[0].startswith('bond') is True or sline[0].startswith('slaves') is True or  sline[0].startswith('mtu') is True:
                if sline[0].startswith('bond'):    
                    opt = sline[0].split('-')
                    sline.pop(0)
                    ifs = " ".join(sline)
                    self.adapters[self._context].replaceBrbond(opt[1], ifs)
                else:
                    opt = sline[0]
                    sline.pop(0)
                    ifs = " ".join(sline)
                    self.adapters[self._context].replaceBrbond(opt, ifs)     
            elif sline[0] == 'up' or sline[0] == 'down' or sline[0] == 'pre-up' or sline[0] == 'post-down' or sline[0] == 'post-up':
                ud = sline.pop(0)
                cmd = ' '.join(sline)
                if ud == 'up':
