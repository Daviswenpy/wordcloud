    ...
    try:
        ...
        if res.get("scores", 1) != 0:
            ...
            if index and query != {}:
                ...
                if temp['forensics'].has_key("graphs"):
                    if 'display' in temp and xrs == 'eas' and 'agg_query' in temp['forensics']['graphs']['template'][0]:
                        aggs_querys = {}
        ...
        if config.UCSS_VERSION == 3.3 and xrs == 'eas' and dct.has_key("graphs"):
            graphs = data[ID]['forensics']["graphs"]
            ...
            for i in _graphs["template"]:
                if i["type"] in [1, 4]:
                    ...
                    try:
                        ...
                        graphs["histSize"] = get_histCnt(graphs["histSize"], timestamp)
                elif i["type"] == 3:
                    try:
                        graphs["timestamp"] = [item["key"] for item in graphs["timestamp"]]
                    except:
                        graphs["timeseries"] = [item["key"] for item in graphs["timeseries"]]

            # 3.3 specific anomaly timestamp >>> datetime
            logs = data[ID]['forensics']['details']['logs']
            if logs[0].has_key('not_valid_before') or logs[0].has_key('not_valid_after'):
                for log in logs:
                    before_old = log['not_valid_before']
                    after_old = log['not_valid_after']
                    log['not_valid_before'] = str(datetime.datetime.fromtimestamp(before_old))[:10]
                    log['not_valid_after'] = str(datetime.datetime.fromtimestamp(after_old))[:10]
...
def get_histCnt(histCnt,timestamp):
    ...
    if not histCnt:
        ...
        histCnt_date_lst = [get_formattime(item["key"]/1000) for item in histCnt]
        match_date = [ j for i in histCnt_date_lst for j in datetime_lst if i == j]
        histCnt_temp = [0] * 7
        for date in match_date:
            histCnt_temp[datetime_lst.index(date)] = histCnt[histCnt_date_lst.index(date)]["doc_count"]
