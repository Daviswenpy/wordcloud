...
device_url = base_url + "devices/" + deviceID_BJ


def print_info(type, verbose, res, url, body=None):
    print "+" + "-" * 100 + "+"
...
class XrsForensics:
    ...
    def post_xrsForensics(self, headers, xrs=None, pk=None, ID=None):
        ...
        body = {"timestamp":1610784000,"user":"172.22.77.222","pageSize":1}
        if xrs == "ars":
            self.url = self.url + "/ars"
            res = requests.post(self.url, headers=headers, json=body)
        elif xrs == "ers":
...
class AnomaliesSummary:
    ...
    def post_anomalies(self, headers):
        body = {"filter": [{"type": 3, "value": ["*"]}], "startTimestamp": 1609862400, "endTimestamp": 1610467200, "pageSize": 20, "page": 1}
...
class TestApi:
    ...
    def test_get_anoalies(self):
        anomalies = EasAnomaly()
        anomalies.get_easAnomaly(self.headers)

    def test_get_ersModels(self):
        ersmodels = ErsModels()
        ersmodels.get_ersModels(self.headers)

    def test_get_scores(self):
        scores = XrsScores()
        scores.get_scores(self.headers,)

    def test_forensics(self):
        forensics = XrsForensics()
        # forensics.post_xrsForensics(self.headers,"ers")
        forensics.post_xrsForensics(self.headers,"eas", "anomaly", "f259af9a-074b-11e9-be98-144f8a006a90")
        # forensics.post_xrsForensics(self.headers,"ers", "model", "d16d0051-c850-442f-82f0-944da9ac214b")
...
if __name__ == '__main__':
    ...
    test = TestApi()
    # test.test_configs()
    # test.test_get_anoalies()
    # test.test_get_ersModels()
    # test.test_get_scores()
    test.test_forensics()
    # test.test_report()
