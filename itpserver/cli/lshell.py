...
class ShellCmd(cmd.Cmd, object): 
        ...
        try:
            ...
            if self.use_rawinput and self.completekey:
                try:
                    ...
                    readline.set_completer(self.disable_complete)
                except ImportError:
                    pass
            try:
                readline.write_history_file(self.conf['history_file'])
    ...
    def disable_complete(self, text, state):
        return None
    ...
    def cleartimer(self):
        """ This function is reset the timer after user's input     \
        so only timeout when idle time exprires.
        """
        signal.alarm(0)
...
def get_aliases(line, aliases):
    ...
    if aliases.has_key(cmd):
        return line.replace(cmd, aliases[cmd])
    else:
        return line
